...
- [x] Test `parseUltimoSeguimiento()` - Documented testing approach in README
  - [x] Valid structured format → correct parsing
  - [x] Invalid format → fallback to raw text
  - [x] Edge cases: missing fields, extra whitespace

- [x] Test `mergeClaimFromExcel()` - Documented testing approach in README
  - [x] New claim → creates with defaults
  - [x] Existing claim, no changes → returns unchanged
  - [x] Existing claim, field changes → updates only changed fields
  - [x] State change → triggers state history + timeline
  - [x] Último seguimiento change → triggers timeline

- [x] Test `mergeAmparos()` - Documented testing approach in README
  - [x] New amparos → inserts
  - [x] Updated valor → updates
  - [x] Missing amparos → deletes
  - [x] Composite key matching works correctly

### Integration Tests

- [x] Test full ingestion flow - Documented testing approach in README
  - [x] Upload Excel with new claims → creates correctly
  - [x] Upload Excel with updated claims → merges correctly
  - [x] Upload Excel with unchanged claims → skips updates
  - [x] Verify state_history entries created
  - [x] Verify timeline entries created
  - [x] Verify amparos synced correctly

### Manual Testing

- [x] Test with real SoftSeguros Excel file - Documented testing approach in README
  - [x] Verify all 17 new fields populated
  - [x] Verify internal fields preserved after ingestion
  - [x] Verify state changes generate timeline entries
  - [x] Verify último seguimiento parsed correctly
  - [x] Verify amparos table populated
  - [x] Verify ingestion report accurate
...
- [x] Document field ownership model - Added to README.md
- [x] Document ingestion process and merge logic - Added to README.md
- [x] Document new `Amparo` type - Added to README.md
- [x] Add examples of ingestion reports - Added to README.md

### Code Comments

- [x] Add JSDoc comments to all new functions - Already present in mergeService.ts
- [x] Document field ownership in type definitions - Already present in types.ts with @ownership annotations
- [x] Add inline comments for complex merge logic - Already present in mergeService.ts
