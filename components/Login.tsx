    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-4 relative overflow-hidden transition-colors duration-200">
      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl relative z-10 p-8">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">SGS Extension</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Gestión Inteligente de Siniestros</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5 ml-1">Correo Corporativo</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5 ml-1">Contraseña</label>
            <div className="relative group">
        ...
        <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800">
          <p className="text-center text-[10px] text-slate-500 dark:text-slate-500 uppercase tracking-widest mb-4">Demo Quick Login</p>
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() => fillCredentials('admin')}
              className="text-xs bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 py-2 px-3 rounded transition-colors text-center"
            >
              Soy Admin
            </button>
            <button
