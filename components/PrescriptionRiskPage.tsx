...
const PrescriptionRiskPage: React.FC<PrescriptionRiskPageProps> = ({
  claims,
  onSelectClaim,
  onBack,
}) => {
    ...
    return claims
      .filter(c => {
        ...
        if (
          ...
          return false;
        const ocurrencia = parseDate(c.fecha_ocurrencia);
        ...
        const daysToPrescription = Math.ceil(
          (prescriptionDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
        );
        ...
        if (filters.tecnico_asignado && c.tecnico_asignado !== filters.tecnico_asignado)
          return false;
      ...
      <div className="flex items-center space-x-4 mb-6">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Gestiona los casos que vencen pronto (Próximos 90 días)
          </p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 space-y-4">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="text-sm text-slate-500 dark:text-slate-400">
            Mostrando{' '}
            <span className="font-bold text-slate-900 dark:text-white">{riskClaims.length}</span>{' '}
            casos críticos
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 pt-2 border-t border-slate-200 dark:border-slate-700/50">
          ...
          >
            ...
            {getUniqueValues('asegurado').map(val => (
          ...
          >
            ...
            {getUniqueValues('aseguradora').map(val => (
              <option key={val} value={val}>
                {val}
              </option>
            ))}
          ...
          >
            ...
            {getUniqueValues('estado_interno').map(val => (
              <option key={val} value={val}>
                {val}
              </option>
            ))}
          ...
          >
            ...
            {getUniqueValues('tecnico_asignado').map(val => (
              <option key={val} value={val}>
                {val}
              </option>
            ))}
          ...
          >
            ...
            {getUniqueValues('vendedor').map(val => (
              <option key={val} value={val}>
                {val}
              </option>
            ))}
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {riskClaims.length === 0 ? (
                ...
                riskClaims.map(claim => {
                  ...
                  const daysToPrescription = Math.ceil(
                    (prescriptionDate.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium border border-transparent bg-${color}-900/30 text-${color}-200 border-${color}-800`}
                        >
