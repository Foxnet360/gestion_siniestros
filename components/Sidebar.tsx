...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, currentUser, onLogout }) => {
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col z-20 transition-all duration-300">
      <div className="p-6 flex items-center space-x-3 border-b border-slate-200 dark:border-slate-800">
        ...
        <div>
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400 font-medium tracking-wide">EXTENSION</span>
        </div>
      </div>
      ...
      <nav className="flex-1 py-6 px-3 space-y-1">
        {visibleItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={`w-5 h-5 ${active ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`} />
