...
import { toDbFormat } from '../lib/dbMapping';
import { addYears, parse } from 'date-fns';
import { normalizeEstado } from '../utils/statusNormalizer';
...
function valuesEqual(excelValue: any, dbValue: any, fieldName: string): boolean {
  ...
  if (currencyFields.includes(fieldName)) {
    ...
    if (!equal && DEBUG_AUDIT) {
      console.log(`  [AUDIT] ${fieldName}: number mismatch`, {
        excel: numA,
        db: numB,
  ...
  if (valA !== valB && DEBUG_AUDIT) {
    console.log(`  [AUDIT] ${fieldName}: strict mismatch`, {
      excel: valA,
      db: valB,
      types: [typeof valA, typeof valB],
    });
  }
  return valA === valB;
}
...
export function parseUltimoSeguimiento(raw: string): TimelineEvent | null {
  ...
  const pattern =
    /Fecha:\s*(\d{2}\/\d{2}\/\d{4})\s*-\s*Funcionario:\s*([^-]+)\s*-\s*Seguimiento:\s*"([^"]+)"\s*(.*)/;
...
export async function handleStateChange(
  ...
  // TODO: Restaurar cuando RLS esté configurado
...
/**
 * Normalize estado_softseguros to extract the final internal state
 */
export function normalizeEstadoSoftseguros(estadoSoftseguros: string | undefined | null): string {
  if (!estadoSoftseguros) return 'AVISO SINIESTRO';
  const parts = estadoSoftseguros.split('-');
  let lastPart = parts[parts.length - 1].trim();
  
  // Strip numbering prefixes like "5. ", "3C. ", "1.\t"
  lastPart = lastPart.replace(/^\d+[a-zA-Z]?\.\s*/, '').trim();
...
export async function mergeClaimFromExcel(
  ...
  if (!existingClaim) {
    ...
    const fechaSiniestro = excelRow.fecha_ocurrencia
      ? new Date(excelRow.fecha_ocurrencia)
      : new Date();
    ...
    const newClaim: Partial<Claim> = {
      ...
      // Internal defaults
      id_interno: `INT-${id}`,
      estado_interno: normalizeEstado(excelRow.estado_softseguros),
      ...
      numero_siniestro: excelRow.numero_siniestro || `SN-${id}`,
      monto_reclamo: excelRow.monto_reclamo || 0,
      valor_deducible: excelRow.valor_deducible || 0,
      valor_indemnizacion: excelRow.valor_indemnizacion || 0,
    };

    // Insert into database (convert to DB format for PostgreSQL)
    const { data, error } = await supabase
      .from('claims')
      .insert(toDbFormat(newClaim))
      .select()
      .single();

    if (error) {
      console.error('[MERGE] Error inserting claim:', {
        id: id,
        error: error,
        claim: newClaim,
      });
      throw new Error(`Error insertando claim ${id}: ${error.message}`);
    }
  ...
  // Detect state change
  if (
    updates.estado_softseguros &&
    updates.estado_softseguros !== existingClaim.estado_softseguros
  ) {
    const nuevoEstadoInterno = normalizeEstado(updates.estado_softseguros as string);
    if (nuevoEstadoInterno !== existingClaim.estado_interno) {
      updates.estado_interno = nuevoEstadoInterno;
      await handleStateChange(
        id,
        existingClaim.estado_interno,
        nuevoEstadoInterno
      );
    }
  }
  ...
  if (
    updates.ultimo_seguimiento_raw &&
    updates.ultimo_seguimiento_raw !== existingClaim.ultimo_seguimiento_raw
  ) {
...
export async function mergeAmparos(
  ...
  if (toDelete.length > 0) {
    const { error } = await supabase.from('amparos').delete().in('id', toDelete);
    if (error) throw error;
    result.deleted = toDelete.length;
  }

  console.log(
    `[AMPAROS] Claim ${claimId} completed: +${result.inserted} inserted, ~${result.updated} updated, -${result.deleted} deleted`
  );
...
export async function ingestClaims(
  ...
  if (amparos.length > 0) {
    ...
    console.log(
      `[INGEST] Unique claim_ids in amparos:`,
      [...new Set(amparos.map(a => a.claim_id))].slice(0, 5)
    );
  }
    ...
    await Promise.all(
      batch.map(async (claim, batchIndex) => {
        ...
        try {
          ...
          if (fetchError) {
            console.error(`[INGEST] Error fetching claim ${id}:`, fetchError);
            throw new Error(`Error fetching claim ${id}: ${fetchError.message}`);
          }
          ...
          console.error(`[INGEST] Error processing claim ${id}:`, err);
          throw err;
        }
      })
    );
  ...
  console.log(
    `  Claims: ${report.claims.created} created, ${report.claims.updated} updated, ${report.claims.unchanged} unchanged`
  );
  console.log(
    `  Amparos: ${report.amparos.inserted} inserted, ${report.amparos.updated} updated, ${report.amparos.deleted} deleted`
  );
