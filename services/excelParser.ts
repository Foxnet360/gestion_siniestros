...
export const processFiles = async (softFile: File, gestionFile?: File): Promise<ParseResult> => {
    ...
    let amparosData: any[] = [];
    const amparosSheetName = workbook.SheetNames.find(name =>
        name.toLowerCase().includes('amparo') || name.toLowerCase().includes('cobertura')
    );
    if (amparosSheetName) {
...
const parseSiniestros = (softData: any[], gestionData: any[]): Partial<Claim>[] => {
    ...
    if (softData.length > 0) {
        console.log('[SINIESTROS] First row columns:', Object.keys(softData[0]));
    ...
    return softData.map((row: any, index: number) => {
        ...
        return {
            ...
            numero_siniestro_compania: parseString(row['NÚMERO SINIESTRO COMPAÑÍA'] || row['NUMERO SINIESTRO COMPANIA']),
            ...
            fecha_notificacion_aseguradora: parseDateOnly(row['FECHA NOTIFICACIÓN ASEGURADORA'] || row['FECHA NOTIFICACION']),
            ...
            celular_principal: parseString(row['CELULAR PRINCIPAL'] || row['CELULAR'] || row['TELEFONO']),
            porcentaje_siniestralidad: parseCurrency(row['PORCENTAJE SINIESTRALIDAD'] || row['% SINIESTRALIDAD']),
            finalizado: row['FINALIZADO'] === 'SI' || row['FINALIZADO'] === true || row['FINALIZADO'] === 1,
            ...
            estado_gestion_softseguros: gestionRow ? parseString(gestionRow['ESTADO GESTION']) : undefined,

            // Internal fields (managed internally, not from Excel)
            tecnico_asignado: gestionRow ? parseString(gestionRow['Responsable']) || 'Sin Asignar' : 'Sin Asignar'
        } as Partial<Claim>;
    }).filter((c): c is Partial<Claim> => c !== null);
};

const parseAmparos = (amparosData: any[], siniestroToIdMap: Map<string, string>): Partial<Amparo>[] => {
    ...
    if (amparosData.length > 0) {
        ...
        console.log('[EXCEL PARSER] First row sample:', {
            ...
            numero: amparosData[0]['NÚMERO SINIESTRO'] || amparosData[0]['NÚMERO DE SINIESTRO'] || amparosData[0]['NUMERO SINIESTRO'],
            reclamante: amparosData[0]['NOMBRE RECLAMANTE'] || amparosData[0]['NOMBRE DEL RECLAMANTE'] || amparosData[0]['RECLAMANTE'],
    ...
    const parsed = amparosData.map((row: any, index: number) => {
        ...
        if (!claimId) {
            unlinkedCount++;
            if (index < 3) {
                console.log(`[EXCEL PARSER] Row ${index}: Cannot find claim for numero_siniestro "${numeroSiniestro}"`);
            }
            return null;
        }
    ...
    console.log(`[EXCEL PARSER] Successfully parsed ${parsed.length} amparos (${linkedCount} linked, ${unlinkedCount} unlinked)`);
    return parsed;
};
